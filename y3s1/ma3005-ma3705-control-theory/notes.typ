...
== DC Gain
- The DC gain is the ratio between the output and the input when all the
    transient responses go away due to the unit-step input.
- It is the steady-state response to the unit-step input, i.e.
    $ "DC gain" = lim_(s -> 0) G(s) $

    Where:
    - $G(s)$ is the transfer function of the closed-loop system.
- It is the static gain in control theory.
...
$
    Delta_k =
    1 - (#text[Loop gains *not touching* the $k^"th"$ forward path])
$
...
Recomputing the steady-state $x_(s s)$ with final value theorem:
$
...
- The responses can be written as:
    ...
    Where:
    - $c_(t r)(t)$ denotes the transient response
    - $c_(s s)(t)$ denotes the steady state response
...
- To obtain the expression of the transient response, expression of the poles of
    $G(s)$ is needed.
- The poles of $G(s)$ are the roots of the denominator polynomial, or the roots
    of:
...
#grid(
    ...
    [
        ...
        When $omega >> a$:
        $ G(s = j omega) = (j omega)/a = omega/a e^(j (pi/2)) $

        *Magnitude*:
        $
            20 lg |G(j omega)| & = 20 lg omega/a \
                               & = 20 lg omega - 20 lg a
        $
