...
#let titlecase(text) = {
    text.replace(regex("\w\S*"), matched => {
        upper(matched.text.at(0)) + lower(matched.text.slice(1))
    })
}
...
- *FIQ*: Entered when a high priority (fast) interrupt is raised. Privilege and
    exception mode.
...
#align(center)[
    ...
    #table(
        ...
        [*`.sect`* "_section name_"],
        [Assembles into a named (initialised) section],
...
a rate that is high enough to capture the changes in the signal.
...
- Transmitting and receiving are both enabled out of reset. Before any control
...
- Ability to determine the elapsed time between the assertion of the timer
...
#let load_store_docs(base_instruction, size_letter) = {
    ...
    if base_instruction == "LDR" {
        [
            - #titlecase(instruction_word_s) a *#memory_type* from the memory
                address of `R1`, and save the contents in register `R4`.
        ]
    } else if base_instruction == "STR" {
        [
