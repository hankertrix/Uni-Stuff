...
; activate clock for ADC0
		...
		LDR R1, =ADC0_ACTSS_R
		LDR R0, [R1]
		BIC R0, R0, #0x0002				; disable SS1 before configuration to
		STR R0, [R1]					; prevent erroneous execution if a trigger event were to occur

		LDR R1, =ADC0_EMUX_R
		LDR R0, [R1]
		BIC R0, R0, #0x00F0				; SS1 is software trigger
		STR R0, [R1]

		LDR R1, =ADC0_SSMUX1_R
		LDR R0, [R1]
		BIC R0, R0, #0x00FF				; clear SS1 field
		...
		LDR R1, =ADC0_IM_R
		LDR R0, [R1]
		BIC R0, R0, #0x0002				; disable SS1 interrupts
		STR R0, [R1]

		LDR R1, =ADC0_ACTSS_R
		LDR R0, [R1]
		ORR R0, R0, #0x0002				; enable SS1
		STR R0, [R1]

Loop
		LDR R1, =ADC0_PSSI_R
		MOV R0, #0x02					; initiate sampling in SS1
		STR R0, [R1]
		...
		LDR R1, =ADC0_ISC_R
		LDR R0, [R1]
		ORR R0, R0, #02					; acknowledge conversion
		STR R0, [R1]
