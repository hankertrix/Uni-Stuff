...
- After the ~def~ keyword comes the name of the function, which has the same rules as Python identifiers. Refer to section [[*Attributes of identifiers in Python]] for the rules.
...
Return the first *"falsy"* value if there are any, else return the *last* value in the expression. Refer to section [[*"Truthy" and "falsy" values]] for an explanation of "falsy".
...
Return the first *"truthy"* value if there are any, else return the *last* value in the expression. Refer to section [[*"Truthy" and "falsy" values]] for an explanation of "truthy".
